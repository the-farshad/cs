...
export const PROBLEMS: Problem[] = [
  ...
  },
  {
    ...
    difficulty: 'medium',
    topic: 'Arrays',
    prompt: 'Return an array `out` where out[i] is the product of every element of `nums` except nums[i]. Solve it without using division.\n\nExample: [1, 2, 3, 4] → [24, 12, 8, 6].',
    funcName: { python: 'product_except_self', javascript: 'productExceptSelf' },
    starter: {
    ...
    solutions: [
      ...
      { language: 'cpp', code: "vector<int> productExceptSelf(vector<int>& nums) {\n    int n = nums.size();\n    vector<int> out(n, 1);\n    int pre = 1;\n    for (int i = 0; i < n; i++) { out[i] = pre; pre *= nums[i]; }\n    int suf = 1;\n    for (int i = n - 1; i >= 0; i--) { out[i] *= suf; suf *= nums[i]; }\n    return out;\n}" },
      { language: 'java', code: "int[] productExceptSelf(int[] nums) {\n    int n = nums.length;\n    int[] out = new int[n];\n    int pre = 1;\n    for (int i = 0; i < n; i++) { out[i] = pre; pre *= nums[i]; }\n    int suf = 1;\n    for (int i = n - 1; i >= 0; i--) { out[i] *= suf; suf *= nums[i]; }\n    return out;\n}" },
      { language: 'pseudocode', code: "function productExceptSelf(nums):\n    out[i] = product of everything before i (prefix pass)\n    multiply out[i] by product of everything after i (suffix pass)\n    return out" },
    ],
  },
  {
    ...
    prompt: 'Return the length of the longest substring of `s` that contains no repeating characters.\n\nExample: "abcabcbb" → 3 (the substring "abc"); "bbbbb" → 1.',
    funcName: { python: 'length_of_longest_substring', javascript: 'lengthOfLongestSubstring' },
    starter: {
    ...
    solutions: [
      ...
      { language: 'cpp', code: "int lengthOfLongestSubstring(string s) {\n    unordered_map<char,int> seen;\n    int best = 0, start = 0;\n    for (int i = 0; i < (int)s.size(); i++) {\n        if (seen.count(s[i]) && seen[s[i]] >= start) start = seen[s[i]] + 1;\n        seen[s[i]] = i;\n        best = max(best, i - start + 1);\n    }\n    return best;\n}" },
      { language: 'java', code: "int lengthOfLongestSubstring(String s) {\n    Map<Character,Integer> seen = new HashMap<>();\n    int best = 0, start = 0;\n    for (int i = 0; i < s.length(); i++) {\n        char c = s.charAt(i);\n        if (seen.containsKey(c) && seen.get(c) >= start) start = seen.get(c) + 1;\n        seen.put(c, i);\n        best = Math.max(best, i - start + 1);\n    }\n    return best;\n}" },
      { language: 'pseudocode', code: "function lengthOfLongestSubstring(s):\n    slide a window [start, i]; remember the last index of each char\n    when a repeat inside the window is seen, move start past it\n    track the largest window size" },
    ],
  },
  {
    slug: 'coin-change',
    ...
    difficulty: 'medium',
    topic: 'Dynamic Programming',
    prompt: 'Given coin denominations `coins` and a target `amount`, return the fewest coins that sum to the amount, or -1 if it is impossible.\n\nExample: ([1, 2, 5], 11) → 3 (5 + 5 + 1); ([2], 3) → -1.',
    funcName: { python: 'coin_change', javascript: 'coinChange' },
    starter: {
    ...
    solutions: [
      ...
      { language: 'cpp', code: "int coinChange(vector<int>& coins, int amount) {\n    int INF = amount + 1;\n    vector<int> dp(amount + 1, INF);\n    dp[0] = 0;\n    for (int a = 1; a <= amount; a++)\n        for (int c : coins)\n            if (c <= a) dp[a] = min(dp[a], dp[a - c] + 1);\n    return dp[amount] == INF ? -1 : dp[amount];\n}" },
      { language: 'java', code: "int coinChange(int[] coins, int amount) {\n    int INF = amount + 1;\n    int[] dp = new int[amount + 1];\n    Arrays.fill(dp, INF);\n    dp[0] = 0;\n    for (int a = 1; a <= amount; a++)\n        for (int c : coins)\n            if (c <= a) dp[a] = Math.min(dp[a], dp[a - c] + 1);\n    return dp[amount] == INF ? -1 : dp[amount];\n}" },
      { language: 'pseudocode', code: "function coinChange(coins, amount):\n    dp[0] = 0; dp[a>0] = infinity\n    for a in 1..amount:\n        for c in coins where c <= a:\n            dp[a] = min(dp[a], dp[a - c] + 1)\n    return dp[amount] if reachable else -1" },
    ],
  },
  {
    ...
    difficulty: 'medium',
    topic: 'Graphs',
    prompt: 'There are `numCourses` courses labelled 0..numCourses-1. Each `prerequisites[i] = [a, b]` means b must be taken before a. Return true if you can finish every course (i.e. there is no cyclic dependency).\n\nExample: (2, [[1, 0]]) → true; (2, [[1, 0], [0, 1]]) → false.',
    funcName: { python: 'can_finish', javascript: 'canFinish' },
    starter: {
    ...
    solutions: [
      { language: 'python', code: "def can_finish(num_courses, prerequisites):\n    from collections import deque\n    indeg = [0] * num_courses\n    adj = [[] for _ in range(num_courses)]\n    for a, b in prerequisites:\n        adj[b].append(a)\n        indeg[a] += 1\n    q = deque(i for i in range(num_courses) if indeg[i] == 0)\n    seen = 0\n    while q:\n        n = q.popleft()\n        seen += 1\n        for m in adj[n]:\n            indeg[m] -= 1\n            if indeg[m] == 0:\n                q.append(m)\n    return seen == num_courses" },
      ...
      { language: 'cpp', code: "bool canFinish(int numCourses, vector<vector<int>>& prerequisites) {\n    vector<int> indeg(numCourses, 0);\n    vector<vector<int>> adj(numCourses);\n    for (auto& p : prerequisites) { adj[p[1]].push_back(p[0]); indeg[p[0]]++; }\n    queue<int> q;\n    for (int i = 0; i < numCourses; i++) if (indeg[i] == 0) q.push(i);\n    int seen = 0;\n    while (!q.empty()) {\n        int n = q.front(); q.pop(); seen++;\n        for (int m : adj[n]) if (--indeg[m] == 0) q.push(m);\n    }\n    return seen == numCourses;\n}" },
      { language: 'java', code: "boolean canFinish(int numCourses, int[][] prerequisites) {\n    int[] indeg = new int[numCourses];\n    List<List<Integer>> adj = new ArrayList<>();\n    for (int i = 0; i < numCourses; i++) adj.add(new ArrayList<>());\n    for (int[] p : prerequisites) { adj.get(p[1]).add(p[0]); indeg[p[0]]++; }\n    Deque<Integer> q = new ArrayDeque<>();\n    for (int i = 0; i < numCourses; i++) if (indeg[i] == 0) q.add(i);\n    int seen = 0;\n    while (!q.isEmpty()) {\n        int n = q.poll(); seen++;\n        for (int m : adj.get(n)) if (--indeg[m] == 0) q.add(m);\n    }\n    return seen == numCourses;\n}" },
      { language: 'pseudocode', code: "function canFinish(numCourses, prerequisites):\n    build a graph; count in-degrees\n    repeatedly remove a course with in-degree 0 (Kahn's topological sort)\n    if every course is removed, there was no cycle -> true" },
    ],
  },
  {
    ...
    difficulty: 'medium',
    topic: 'Two Pointers',
    prompt: 'Vertical lines have heights `height`. Pick two lines that, with the x-axis, hold the most water: area = min(height[i], height[j]) × (j - i). Return that maximum area.\n\nExample: [1, 8, 6, 2, 5, 4, 8, 3, 7] → 49.',
    funcName: { python: 'max_area', javascript: 'maxArea' },
    starter: {
    ...
    solutions: [
      ...
      { language: 'cpp', code: "int maxArea(vector<int>& height) {\n    int i = 0, j = height.size() - 1, best = 0;\n    while (i < j) {\n        best = max(best, min(height[i], height[j]) * (j - i));\n        if (height[i] < height[j]) i++; else j--;\n    }\n    return best;\n}" },
      { language: 'java', code: "int maxArea(int[] height) {\n    int i = 0, j = height.length - 1, best = 0;\n    while (i < j) {\n        best = Math.max(best, Math.min(height[i], height[j]) * (j - i));\n        if (height[i] < height[j]) i++; else j--;\n    }\n    return best;\n}" },
      { language: 'pseudocode', code: "function maxArea(height):\n    two pointers at both ends\n    record area; move the shorter side inward (it limits the area)\n    repeat until they meet" },
    ],
  },
  {
    slug: 'house-robber',
    ...
    difficulty: 'medium',
    topic: 'Dynamic Programming',
    prompt: 'Each `nums[i]` is the money in house i. You cannot rob two adjacent houses. Return the maximum you can rob.\n\nExample: [2, 7, 9, 3, 1] → 12 (rob houses 0, 2, 4).',
    funcName: { python: 'rob', javascript: 'rob' },
    starter: {
      python: 'def rob(nums):\n    # your code here\n    pass',
    ...
    solutions: [
      { language: 'python', code: "def rob(nums):\n    prev, cur = 0, 0\n    for n in nums:\n        prev, cur = cur, max(cur, prev + n)\n    return cur" },
      { language: 'javascript', code: "function rob(nums) {\n  let prev = 0, cur = 0;\n  for (const n of nums) { const t = Math.max(cur, prev + n); prev = cur; cur = t; }\n  return cur;\n}" },
      { language: 'cpp', code: "int rob(vector<int>& nums) {\n    int prev = 0, cur = 0;\n    for (int n : nums) { int t = max(cur, prev + n); prev = cur; cur = t; }\n    return cur;\n}" },
      { language: 'java', code: "int rob(int[] nums) {\n    int prev = 0, cur = 0;\n    for (int n : nums) { int t = Math.max(cur, prev + n); prev = cur; cur = t; }\n    return cur;\n}" },
      { language: 'pseudocode', code: "function rob(nums):\n    cur = best using houses up to i; prev = best up to i-1\n    for each n: (prev, cur) = (cur, max(cur, prev + n))\n    return cur" },
    ],
  },
  {
    slug: 'jump-game',
    ...
    difficulty: 'medium',
    topic: 'Greedy',
    prompt: 'Each `nums[i]` is the maximum jump length from index i. Starting at index 0, return true if you can reach the last index.\n\nExample: [2, 3, 1, 1, 4] → true; [3, 2, 1, 0, 4] → false.',
    funcName: { python: 'can_jump', javascript: 'canJump' },
    starter: {
    ...
    solutions: [
      ...
      { language: 'cpp', code: "bool canJump(vector<int>& nums) {\n    int reach = 0;\n    for (int i = 0; i < (int)nums.size(); i++) {\n        if (i > reach) return false;\n        reach = max(reach, i + nums[i]);\n    }\n    return true;\n}" },
      { language: 'java', code: "boolean canJump(int[] nums) {\n    int reach = 0;\n    for (int i = 0; i < nums.length; i++) {\n        if (i > reach) return false;\n        reach = Math.max(reach, i + nums[i]);\n    }\n    return true;\n}" },
      { language: 'pseudocode', code: "function canJump(nums):\n    reach = 0\n    for each index i: if i > reach return false; reach = max(reach, i + nums[i])\n    return true" },
    ],
  },
  {
    slug: 'three-sum',
    ...
    difficulty: 'medium',
    topic: 'Two Pointers',
    prompt: 'Return all unique triplets [a, b, c] from `nums` with a + b + c = 0. The result is sorted, and each triplet is in ascending order.\n\nExample: [-1, 0, 1, 2, -1, -4] → [[-1, -1, 2], [-1, 0, 1]].',
    funcName: { python: 'three_sum', javascript: 'threeSum' },
    starter: {
    ...
    solutions: [
      ...
      { language: 'cpp', code: "vector<vector<int>> threeSum(vector<int>& nums) {\n    sort(nums.begin(), nums.end());\n    vector<vector<int>> res;\n    int n = nums.size();\n    for (int i = 0; i < n; i++) {\n        if (i > 0 && nums[i] == nums[i - 1]) continue;\n        int l = i + 1, r = n - 1;\n        while (l < r) {\n            int s = nums[i] + nums[l] + nums[r];\n            if (s < 0) l++;\n            else if (s > 0) r--;\n            else {\n                res.push_back({nums[i], nums[l], nums[r]});\n                l++; r--;\n                while (l < r && nums[l] == nums[l - 1]) l++;\n                while (l < r && nums[r] == nums[r + 1]) r--;\n            }\n        }\n    }\n    return res;\n}" },
      { language: 'java', code: "List<List<Integer>> threeSum(int[] nums) {\n    Arrays.sort(nums);\n    List<List<Integer>> res = new ArrayList<>();\n    int n = nums.length;\n    for (int i = 0; i < n; i++) {\n        if (i > 0 && nums[i] == nums[i - 1]) continue;\n        int l = i + 1, r = n - 1;\n        while (l < r) {\n            int s = nums[i] + nums[l] + nums[r];\n            if (s < 0) l++;\n            else if (s > 0) r--;\n            else {\n                res.add(Arrays.asList(nums[i], nums[l], nums[r]));\n                l++; r--;\n                while (l < r && nums[l] == nums[l - 1]) l++;\n                while (l < r && nums[r] == nums[r + 1]) r--;\n            }\n        }\n    }\n    return res;\n}" },
      { language: 'pseudocode', code: "function threeSum(nums):\n    sort nums\n    for each i: two-pointer search the rest for a pair summing to -nums[i]\n    skip duplicates to keep triplets unique" },
    ],
  },
  {
    ...
    difficulty: 'medium',
    topic: 'Intervals',
    prompt: 'Given `intervals` where each element is [start, end], merge all overlapping intervals and return them sorted by start.\n\nExample: [[1, 3], [2, 6], [8, 10], [15, 18]] → [[1, 6], [8, 10], [15, 18]].',
    funcName: { python: 'merge', javascript: 'merge' },
    starter: {
    ...
    solutions: [
      ...
      { language: 'cpp', code: "vector<vector<int>> merge(vector<vector<int>>& intervals) {\n    sort(intervals.begin(), intervals.end());\n    vector<vector<int>> res;\n    for (auto& iv : intervals) {\n        if (!res.empty() && iv[0] <= res.back()[1]) res.back()[1] = max(res.back()[1], iv[1]);\n        else res.push_back(iv);\n    }\n    return res;\n}" },
      { language: 'java', code: "int[][] merge(int[][] intervals) {\n    Arrays.sort(intervals, (a, b) -> a[0] - b[0]);\n    List<int[]> res = new ArrayList<>();\n    for (int[] iv : intervals) {\n        if (!res.isEmpty() && iv[0] <= res.get(res.size() - 1)[1])\n            res.get(res.size() - 1)[1] = Math.max(res.get(res.size() - 1)[1], iv[1]);\n        else res.add(iv);\n    }\n    return res.toArray(new int[0][]);\n}" },
      { language: 'pseudocode', code: "function merge(intervals):\n    sort by start\n    for each interval: if it overlaps the last kept one, extend its end; else append it\n    return kept intervals" },
    ],
  },
  {
    ...
    difficulty: 'hard',
    topic: 'Two Pointers',
    prompt: 'Given an elevation map `height` (width 1 per bar), return how many units of rain water it can trap.\n\nExample: [0, 1, 0, 2, 1, 0, 1, 3, 2, 1, 2, 1] → 6.',
    funcName: { python: 'trap', javascript: 'trap' },
    starter: {
    ...
    solutions: [
      ...
      { language: 'javascript', code: "function trap(height) {\n  let l = 0, r = height.length - 1, lm = 0, rm = 0, total = 0;\n  while (l < r) {\n    if (height[l] < height[r]) { lm = Math.max(lm, height[l]); total += lm - height[l]; l++; }\n    else { rm = Math.max(rm, height[r]); total += rm - height[r]; r--; }\n  }\n  return total;\n}" },
      { language: 'cpp', code: "int trap(vector<int>& height) {\n    int l = 0, r = (int)height.size() - 1, lm = 0, rm = 0, total = 0;\n    while (l < r) {\n        if (height[l] < height[r]) { lm = max(lm, height[l]); total += lm - height[l]; l++; }\n        else { rm = max(rm, height[r]); total += rm - height[r]; r--; }\n    }\n    return total;\n}" },
      { language: 'java', code: "int trap(int[] height) {\n    int l = 0, r = height.length - 1, lm = 0, rm = 0, total = 0;\n    while (l < r) {\n        if (height[l] < height[r]) { lm = Math.max(lm, height[l]); total += lm - height[l]; l++; }\n        else { rm = Math.max(rm, height[r]); total += rm - height[r]; r--; }\n    }\n    return total;\n}" },
      { language: 'pseudocode', code: "function trap(height):\n    two pointers; track max-so-far from each side\n    water above a bar = (lower of the two running maxes) - its height\n    advance the side with the smaller running max" },
    ],
  },
  {
    slug: 'edit-distance',
    ...
    difficulty: 'hard',
    topic: 'Dynamic Programming',
    prompt: 'Return the minimum number of single-character insertions, deletions, or substitutions needed to turn `word1` into `word2` (the Levenshtein distance).\n\nExample: ("horse", "ros") → 3.',
    funcName: { python: 'min_distance', javascript: 'minDistance' },
    starter: {
    ...
    solutions: [
      ...
      { language: 'cpp', code: "int minDistance(string word1, string word2) {\n    int m = word1.size(), n = word2.size();\n    vector<vector<int>> dp(m + 1, vector<int>(n + 1, 0));\n    for (int i = 0; i <= m; i++) dp[i][0] = i;\n    for (int j = 0; j <= n; j++) dp[0][j] = j;\n    for (int i = 1; i <= m; i++)\n        for (int j = 1; j <= n; j++)\n            dp[i][j] = word1[i-1] == word2[j-1] ? dp[i-1][j-1]\n                : 1 + min({dp[i-1][j], dp[i][j-1], dp[i-1][j-1]});\n    return dp[m][n];\n}" },
      { language: 'java', code: "int minDistance(String word1, String word2) {\n    int m = word1.length(), n = word2.length();\n    int[][] dp = new int[m + 1][n + 1];\n    for (int i = 0; i <= m; i++) dp[i][0] = i;\n    for (int j = 0; j <= n; j++) dp[0][j] = j;\n    for (int i = 1; i <= m; i++)\n        for (int j = 1; j <= n; j++)\n            dp[i][j] = word1.charAt(i-1) == word2.charAt(j-1) ? dp[i-1][j-1]\n                : 1 + Math.min(dp[i-1][j], Math.min(dp[i][j-1], dp[i-1][j-1]));\n    return dp[m][n];\n}" },
      { language: 'pseudocode', code: "function minDistance(word1, word2):\n    dp[i][j] = edit distance between first i chars and first j chars\n    if chars match: carry the diagonal; else 1 + min(insert, delete, replace)\n    return dp[m][n]" },
    ],
  },
  {
    slug: 'word-break',
    ...
    difficulty: 'medium',
    topic: 'Dynamic Programming',
    prompt: 'Given a string `s` and a dictionary `wordDict`, return true if `s` can be split into a sequence of one or more dictionary words.\n\nExample: ("applepenapple", ["apple", "pen"]) → true.',
    funcName: { python: 'word_break', javascript: 'wordBreak' },
    starter: {
    ...
    solutions: [
      ...
      { language: 'cpp', code: "bool wordBreak(string s, vector<string>& wordDict) {\n    unordered_set<string> words(wordDict.begin(), wordDict.end());\n    int n = s.size();\n    vector<bool> dp(n + 1, false);\n    dp[0] = true;\n    for (int i = 1; i <= n; i++)\n        for (int j = 0; j < i; j++)\n            if (dp[j] && words.count(s.substr(j, i - j))) { dp[i] = true; break; }\n    return dp[n];\n}" },
      { language: 'java', code: "boolean wordBreak(String s, List<String> wordDict) {\n    Set<String> words = new HashSet<>(wordDict);\n    int n = s.length();\n    boolean[] dp = new boolean[n + 1];\n    dp[0] = true;\n    for (int i = 1; i <= n; i++)\n        for (int j = 0; j < i; j++)\n            if (dp[j] && words.contains(s.substring(j, i))) { dp[i] = true; break; }\n    return dp[n];\n}" },
      { language: 'pseudocode', code: "function wordBreak(s, wordDict):\n    dp[i] = can the first i chars be segmented?\n    dp[0] = true; dp[i] = true if some dp[j] is true and s[j..i] is a word\n    return dp[length(s)]" },
    ],
  },
];
